...
/**
 * Standalone AVX2 UTF-8 case-insensitive substring search.
 *
 * Extracted from StringZilla 5.0.5. Performs full, locale-independent
 * Unicode case folding, including one-to-many folds, and treats malformed
 * UTF-8 bytes losslessly. Requires AVX2.
 *
...
typedef union { __m128i xmm; } sz_u128_vec_t;
typedef union { __m256i ymm; } sz_u256_vec_t;
...
SZ_API_COMPTIME sz_cptr_t sz_find_serial(sz_cptr_t haystack, sz_size_t haystack_length, sz_cptr_t needle,
    ...
    // Plan 9 assembly; gocc cannot translate an indirect CALL target.
...
typedef struct {
    sz_size_t haystack_matched_offset;
    sz_size_t haystack_matched_length;
    sz_size_t needle_head_bytes;
    sz_size_t needle_tail_bytes;
} sz_utf8_uncased_verify_params_t;

SZ_HELPER_AUTO sz_utf8_uncased_verify_result_t sz_utf8_uncased_verify_match_( //
    sz_cptr_t haystack, sz_size_t haystack_length,                           //
    sz_cptr_t needle, sz_size_t needle_length,                               //
    sz_utf8_uncased_verify_params_t const *params) {

    sz_utf8_uncased_verify_result_t result = {SZ_NULL_CHAR, 0};
    sz_size_t const haystack_matched_offset = params->haystack_matched_offset;
    sz_size_t const haystack_matched_length = params->haystack_matched_length;
    sz_size_t const needle_head_bytes = params->needle_head_bytes;
    sz_size_t const needle_tail_bytes = params->needle_tail_bytes;
...
SZ_HELPER_INLINE sz_cptr_t sz_utf8_uncased_search_in_danger_zone_( //
    ...
    while (danger_cursor < danger_end) {
        ...
        if (haystack_folded_runes[0] == needle_first_safe_folded_rune) {
            ...
            sz_utf8_uncased_verify_params_t const verify_params = {
            ...
            sz_utf8_uncased_verify_result_t verified = sz_utf8_uncased_verify_match_( //
                ...
                &verify_params);                             // Verify everything after head serially
...
/** @brief First N bits set, with the full-width case handled without an undefined shift. */
SZ_HELPER_INLINE sz_u32_t sz_utf8_uncased_haswell_mask_until_(sz_size_t n) {
    return n >= 32 ? 0xFFFFFFFFu : (((sz_u32_t)1u << (unsigned)n) - 1u);
}
...
SZ_HELPER_AUTO sz_cptr_t sz_utf8_uncased_search_haswell( //
    sz_cptr_t haystack, sz_size_t haystack_length,        //
    sz_cptr_t needle, sz_size_t needle_length,            //
...
/** Finds the first full-Unicode case-insensitive UTF-8 match by byte offset. */
// gocc: indexFoldAVX2(haystack string, needle string, metadata *indexFoldNeedleMetadata, matched_length *uint64) int
int64_t index_utf8_uncased_avx2(unsigned char *haystack, uint64_t haystack_length,
                                unsigned char *needle, uint64_t needle_length,
    ...
    if (haystack_length == 0) return -1;
    if (haystack_length > (uint64_t)INT64_MAX || needle_length > (uint64_t)INT64_MAX) return -1;

    match = sz_utf8_uncased_search_haswell((sz_cptr_t)haystack, (sz_size_t)haystack_length, (sz_cptr_t)needle,
                                           (sz_size_t)needle_length, needle_metadata, (sz_size_t *)matched_length);
